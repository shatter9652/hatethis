...
static char* shellQuote(const char* s) {
    if (s == nullptr) return safeStrdup("''");
    size_t len = 2; // quotes
...
static char* joinPath2(const char* a, const char* b) {
    if (a == nullptr || a[0] == '\0') return safeStrdup(b != nullptr ? b : "");
    if (b == nullptr || b[0] == '\0') return safeStrdup(a);
    size_t alen = strlen(a);
    bool needsSlash = a[alen - 1] != '/' && a[alen - 1] != '\\';
    char* out = (char*) safeMalloc(alen + (needsSlash ? 1 : 0) + strlen(b) + 1);
    strcpy(out, a);
    if (needsSlash) strcat(out, "/");
    strcat(out, b);
    return out;
}
...
static RValue builtin_game_change(VMContext* ctx, RValue* args, int32_t argCount) {
    ...
    char* launchParameters = (argCount >= 2) ? RValue_toString(args[1]) : safeStrdup("");
...
#ifdef PLATFORM_PS2
    // PS2 does not have a normal cwd/bundle folder. CONFIG.JSN maps exact game
    // paths to host:/ or cdrom0:/ paths, so resolve chapter DATA.WIN through the
    // active FileSystem instead of trying to fopen ./chapterN_windows/data.win.
    char* ps2GameDir = safeStrdup(workingDirectory);
    for (size_t i = 0; ps2GameDir[i] != '\0'; i++) {
        if (ps2GameDir[i] == '\\') ps2GameDir[i] = '/';
    }
    while (ps2GameDir[0] == '/' || (ps2GameDir[0] == '.' && ps2GameDir[1] == '/')) {
        if (ps2GameDir[0] == '/') memmove(ps2GameDir, ps2GameDir + 1, strlen(ps2GameDir));
        else memmove(ps2GameDir, ps2GameDir + 2, strlen(ps2GameDir) - 1);
    }
    size_t ps2GameDirLen = strlen(ps2GameDir);
    while (ps2GameDirLen > 0 && ps2GameDir[ps2GameDirLen - 1] == '/') {
        ps2GameDir[--ps2GameDirLen] = '\0';
    }

    free(chapterDir);
    chapterDir = safeStrdup(ps2GameDir);
    free(childDataWin);
    childDataWin = joinPath2(ps2GameDir, "DATA.WIN");

    if (ctx != nullptr && ctx->runner != nullptr && ((Runner*) ctx->runner)->fileSystem != nullptr &&
        ((Runner*) ctx->runner)->fileSystem->vtable->resolvePath != nullptr) {
        char* resolved = ((Runner*) ctx->runner)->fileSystem->vtable->resolvePath(((Runner*) ctx->runner)->fileSystem, childDataWin);
        if (resolved != nullptr) {
            free(childDataWin);
            childDataWin = resolved;
        } else {
            fprintf(stderr, "VM: PS2 game_change failed to resolve %s through CONFIG.JSN mappings\n", childDataWin);
        }
    }
    setenv("BUTTERSCOTCH_PS2_ACTIVE_GAMEDIR", ps2GameDir, 1);
    free(ps2GameDir);
#endif
...
static RValue builtin_parameter_count(MAYBE_UNUSED VMContext* ctx, MAYBE_UNUSED RValue* args, MAYBE_UNUSED int32_t argCount) {
    ...
    if (count == nullptr) return RValue_makeReal(0);
    return RValue_makeReal((double) atoi(count));
}

static RValue builtin_parameter_string(MAYBE_UNUSED VMContext* ctx, RValue* args, int32_t argCount) {
    if (argCount < 1) return RValue_makeOwnedString(safeStrdup(""));

    int index = (int) RValue_toReal(args[0]);
    if (index < 1) return RValue_makeOwnedString(safeStrdup(""));

    char envName[64];
    snprintf(envName, sizeof(envName), "BUTTERSCOTCH_PARAMETER_%d", index);
    const char* value = getenv(envName);
    return RValue_makeOwnedString(safeStrdup(value != nullptr ? value : ""));
}

...
typedef struct {
    ...
    const char* gameDir; // DELTARUNNER compatibility: accepted but dataWinPath already selects scope
...
static void parseCommandLineArgs(CommandLineArgs* args, int argc, char* argv[]) {
    ...
    static struct option longOptions[] = {
        ...
        {"gamedir", required_argument, nullptr, 'm'},
#ifdef ENABLE_VM_OPCODE_PROFILER
    ...
    // GameMaker-compatible launch parameters for games that call
    // parameter_count()/parameter_string().  DELTARUNNER passes runner-only
    // options such as -gamedir after data.win; skip those so GML only sees
    // the actual game launch payload (for example: -game data.win launcher ...).
    int gmParamCount = 0;
    for (int i = optind + 1; i < argc; i++) {
        if ((strcmp(argv[i], "-gamedir") == 0 || strcmp(argv[i], "--gamedir") == 0) && i + 1 < argc) {
            i++;
            continue;
        }
        char envName[64];
        snprintf(envName, sizeof(envName), "BUTTERSCOTCH_PARAMETER_%d", gmParamCount + 1);
        setenv(envName, argv[i], 1);
        gmParamCount++;
    }
    char gmParamCountBuf[32];
    snprintf(gmParamCountBuf, sizeof(gmParamCountBuf), "%d", gmParamCount);
    setenv("BUTTERSCOTCH_PARAMETER_COUNT", gmParamCountBuf, 1);

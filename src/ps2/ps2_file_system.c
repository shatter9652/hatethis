...
// DELTARUNE game_change on PS2 keeps one ELF alive but swaps DATA.WIN.
// Unlike desktop OverlayFileSystem, the PS2 filesystem uses explicit CONFIG.JSN
// mappings, so chapter-local reads like "lang/lang_ja.json" must be scoped to
// "chapter3_windows/lang/lang_ja.json" after game_change.
static char* ps2NormalizeScopedKey(const char* activeDir, const char* relativePath) {
    if (activeDir == nullptr || activeDir[0] == '\0' || relativePath == nullptr || relativePath[0] == '\0') return nullptr;

    while (*activeDir == '/' || *activeDir == '\\' || *activeDir == '.') {
        if (*activeDir == '.' && (activeDir[1] == '/' || activeDir[1] == '\\')) {
            activeDir += 2;
            continue;
        }
    ...
    while (*relativePath == '/' || *relativePath == '\\' || *relativePath == '.') {
        if (*relativePath == '.' && (relativePath[1] == '/' || relativePath[1] == '\\')) {
            relativePath += 2;
            continue;
        }
    ...
    size_t activeLen = strlen(activeDir);
    while (activeLen > 0 && (activeDir[activeLen - 1] == '/' || activeDir[activeLen - 1] == '\\')) activeLen--;
    if (activeLen == 0) return nullptr;

    // Shared root music must never be scoped into chapterN_windows.
    if (strncmp(relativePath, "mus/", 4) == 0 || strncmp(relativePath, "mus\\", 4) == 0) return nullptr;

    size_t relLen = strlen(relativePath);
    char* key = safeMalloc(activeLen + 1 + relLen + 1);
    memcpy(key, activeDir, activeLen);
    key[activeLen] = '/';
    memcpy(key + activeLen + 1, relativePath, relLen + 1);
...
static ptrdiff_t ps2FindMappingIndex(Ps2FileSystem* pfs, const char* relativePath, char** outMappedKey) {
    if (outMappedKey != nullptr) *outMappedKey = nullptr;

    const char* activeDir = getenv("BUTTERSCOTCH_PS2_ACTIVE_GAMEDIR");
    if (activeDir == nullptr || activeDir[0] == '\0') {
        activeDir = getenv("BUTTERSCOTCH_ACTIVE_BUNDLE_DIR");
    }

    // After game_change, prefer chapter-local generated assets and loose files.
    // Without this, a chapter load asks for e.g. TEXTURES.BIN/SOUNDS.BIN and
    // gets the root chapter-select assets, which leaves the game stuck on the
    // Deltarune initializing screen. Save files still fall back to their direct
    // mc0:/ mappings because no scoped mapping exists for them.
    char* scoped = ps2NormalizeScopedKey(activeDir, relativePath);
    if (scoped != nullptr) {
        ptrdiff_t scopedIdx = shgeti(pfs->mappings, scoped);
        if (scopedIdx >= 0) {
            BSC_debugLog("path-ps2", "scoped relative=%s -> %s", relativePath, scoped);
            if (outMappedKey != nullptr) {
                *outMappedKey = scoped;
            } else {
                free(scoped);
            }
...
static char* resolvePath(FileSystem* fs, const char* relativePath) {
    ...
    ptrdiff_t idx = ps2FindMappingIndex(pfs, relativePath, nullptr);
    if (0 > idx) {
